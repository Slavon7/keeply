...
export function DeleteConfirmModal({ title, isBulk = false, isHistoryOnly = false, t, onConfirm, onCancel }: Props) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm"
      ...
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-900 dark:border dark:border-gray-800"
        ...
        <div className={`mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full ${
          isHistoryOnly ? 'bg-yellow-100 dark:bg-yellow-900/30' : 'bg-red-100 dark:bg-red-900/30'
        }`}>
          {isHistoryOnly ? (
        ...
        <h2 className="text-center text-base font-semibold text-gray-900 dark:text-gray-100">
          {isHistoryOnly ? t.delete_history_title : isBulk ? t.delete_bulk_title : t.delete_title}
        </h2>
        ...
        <div className="mt-6 flex gap-3">
          ...
          <button onClick={onConfirm}
            className={`flex-1 rounded-xl py-2.5 text-sm font-medium text-white transition-all active:scale-95 ${
              isHistoryOnly
                ? 'bg-yellow-500 hover:bg-yellow-600'
                : 'bg-red-600 hover:bg-red-700'
            }`}>
            {isHistoryOnly ? t.delete_history_confirm : t.delete_confirm}
          </button>
        </div>
      </div>
    </div>
